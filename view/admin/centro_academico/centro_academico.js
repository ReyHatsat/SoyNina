//-----------------------Variables grobales-----------------------//
let g__centro_academico = [];
let g__centro_academico_datatables
let g__Active = 0;
...
init();
async function init() {
  g__centro_academico = await cargar();
  g__centro_academico = g__centro_academico.document.records;
  renderDataTable();
}
//-----------------------init-----------------------//
...
//-----------------------Creación de objeto-----------------------//
  function generarObjeto(){
    return{
        centro_academico:$("#f_nombre_centro_academico").val(),
        activo:1,
    }
  }
  function generarObjetoEdit(data){
      return {
          id_centro_academico:data,/////////////cambiar/////////////
          centro_academico:$("#f_edit").val(),
          activo:1,
      }
    };
    ...
    function generarObjetoActivar(data){
      ...
      return {
          id_centro_academico:arrayID.id_centro_academico,/////////////cambiar/////////////
          centro_academico:arrayID.centro_academico,/////////////cambiar/////////////
          activo:activoTmp,
        }
      };
...
$("#btn_cancelar_centro_academico").click(function(){
  $("#f_nombre_centro_academico").val("");
});
$("#btn_agregar_centro_academico").click(function(){
  let obj = generarObjeto()
  eventoCrear(obj);
  $("#f_nombre_centro_academico").val("");
});
$("#f_nombre_centro_academico").keyup(function(){/////////////cambiar/////////////
  if ($("#f_nombre_centro_academico").val() == "") {
    $( "#btn_agregar_centro_academico" ).prop( "disabled", true );/////////////cambiar/////////////
  }else {
    $( "#btn_agregar_centro_academico" ).prop( "disabled", false );/////////////cambiar/////////////
  }
});
//-------------------------------->>
function buscarPorID(data) {
  const found = g__centro_academico.find(element => element.id_centro_academico == data);/////////////cambiar/////////////
  return found;
};
function editar(data) {
  $("#contenedor_del_modal").html(`
      <label class="form-label">Nombre del centro academico</label>
    ...
    `);
  let arrayID = buscarPorID(data);
  $("#f_edit").val(arrayID.centro_academico);/////////////cambiar/////////////
};
function activar(data) {
  ...
  $("#contenedor_del_modal").html(`
      <p class="text-danger">¿Está seguro que desea ${estado} el centro academico: ${arrayID.centro_academico}?</p>
...
function renderDataTable() {
  g__centro_academico_datatables = $("#dt").DataTable({/////////////cambiar/////////////
    data: g__centro_academico,/////////////cambiar/////////////
    response: true,
    destroy: true,
    columns: [/////////////cambiar/////////////
      {data: "id_centro_academico"},
      {data: "centro_academico"},
      {
        data: 'activo',
...
function cargar(){
    return new Promise( resolve => {
        fetch('API/V1/centro_academico/read.php')/////////////cambiar/////////////
        .then( r => r.json() )
        .then( r => resolve(r) )
    })
}
...
function eventoCrear(obj){
    fetch('API/V1/centro_academico/create.php', {/////////////cambiar/////////////
        method:'POST',
...
function eventoEditar(data){
    ...
    fetch(`API/V1/centro_academico/update.php?id_centro_academico=${data}`, {/////////////cambiar/////////////
        method:'POST',
