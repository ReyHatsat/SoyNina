//-----------------------Variables grobales-----------------------//
let g__Tipo_convivencia = [];
let g__Tipo_convivencia_datatables
let g__Active = 0;
...
init();
async function init() {
  g__Tipo_convivencia = await cargar();
  g__Tipo_convivencia = g__Tipo_convivencia.document.records;
  renderDataTable();
}
//-----------------------init-----------------------//
...
//-----------------------Creación de objeto-----------------------//
  function generarObjeto(){
    return{
        tipo_convivencia:$("#f_nombre_tipo_convivencia").val(),
        activo:1,
    }
  }
  function generarObjetoEdit(data){
      return {
          id_tipo_convivencia:data,/////////////cambiar/////////////
          tipo_convivencia:$("#f_edit").val(),
          activo:1,
      }
    };
    ...
    function generarObjetoActivar(data){
      ...
      return {
          id_tipo_convivencia:arrayID.id_tipo_convivencia,/////////////cambiar/////////////
          tipo_convivencia:arrayID.tipo_convivencia,/////////////cambiar/////////////
          activo:activoTmp,
        }
      };
...
$("#btn_cancelar_tipo_convivencia").click(function(){
  $("#f_nombre_tipo_convivencia").val("");
});
$("#btn_agregar_tipo_convivencia").click(function(){
  let obj = generarObjeto()
  eventoCrear(obj);
  $("#f_nombre_tipo_convivencia").val("");
});
$("#f_nombre_tipo_convivencia").keyup(function(){/////////////cambiar/////////////
  if ($("#f_nombre_tipo_convivencia").val() == "") {
    $( "#btn_agregar_tipo_convivencia" ).prop( "disabled", true );/////////////cambiar/////////////
  }else {
    $( "#btn_agregar_tipo_convivencia" ).prop( "disabled", false );/////////////cambiar/////////////
  }
});
//-------------------------------->>
function buscarPorID(data) {
  const found = g__Tipo_convivencia.find(element => element.id_tipo_convivencia == data);/////////////cambiar/////////////
  return found;
};
function editar(data) {
  $("#contenedor_del_modal").html(`
      <label class="form-label">Nombre del tipo de convivencia</label>
    ...
    `);
  let arrayID = buscarPorID(data);
  $("#f_edit").val(arrayID.tipo_convivencia);/////////////cambiar/////////////
};
function activar(data) {
  ...
  $("#contenedor_del_modal").html(`
      <p class="text-danger">¿Está seguro que desea ${estado} el tipo de convivencia: ${arrayID.tipo_convivencia}?</p>
...
function renderDataTable() {
  g__Tipo_convivencia_datatables = $("#dt").DataTable({/////////////cambiar/////////////
    data: g__Tipo_convivencia,/////////////cambiar/////////////
    response: true,
    destroy: true,
    columns: [/////////////cambiar/////////////
      {data: "id_tipo_convivencia"},
      {data: "tipo_convivencia"},
      {
        data: 'activo',
...
function cargar(){
    return new Promise( resolve => {
        fetch('API/V1/tipo_convivencia/read.php')/////////////cambiar/////////////
        .then( r => r.json() )
        .then( r => resolve(r) )
    })
}
...
function eventoCrear(obj){
    fetch('API/V1/tipo_convivencia/create.php', {/////////////cambiar/////////////
        method:'POST',
...
function eventoEditar(data){
    ...
    fetch(`API/V1/tipo_convivencia/update.php?id_tipo_convivencia=${data}`, {/////////////cambiar/////////////
        method:'POST',
