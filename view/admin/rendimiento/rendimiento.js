...
init();
async function init() {
  g__rendimiento = await cargar();
  g__rendimiento = g__rendimiento.document.records;
  renderDataTable();
}
//-----------------------init-----------------------//
...
//-----------------------Creación de objeto-----------------------//
  function generarObjeto(){
    return{
        rendimiento:$("#f_nombre_rendimiento").val(),
        activo:1,
    }
  }
  function generarObjetoEdit(data){
      return {
          id_rendimiento:data,/////////////cambiar/////////////
          rendimiento:$("#f_edit").val(),
          activo:1,
      }
    };
    ...
    function generarObjetoActivar(data){
      ...
      return {
          id_rendimiento:arrayID.id_rendimiento,/////////////cambiar/////////////
          rendimiento:arrayID.rendimiento,/////////////cambiar/////////////
          activo:activoTmp,
        }
      };
...
$("#btn_cancelar_rendimiento").click(function(){
  $("#f_nombre_rendimiento").val("");
});
$("#btn_agregar_rendimiento").click(function(){
  let obj = generarObjeto()
  eventoCrear(obj);
  $("#f_nombre_rendimiento").val("");
});
$("#f_nombre_rendimiento").keyup(function(){/////////////cambiar/////////////
  if ($("#f_nombre_rendimiento").val() == "") {
    $( "#btn_agregar_rendimiento" ).prop( "disabled", true );/////////////cambiar/////////////
  }else {
    $( "#btn_agregar_rendimiento" ).prop( "disabled", false );/////////////cambiar/////////////
  }
});
//-------------------------------->>
function buscarPorID(data) {
  const found = g__rendimiento.find(element => element.id_rendimiento == data);/////////////cambiar/////////////
  return found;
};
function editar(data) {
  $("#contenedor_del_modal").html(`
      <label class="form-label">Nombre del rendimiento</label>
    ...
    `);
  let arrayID = buscarPorID(data);
  $("#f_edit").val(arrayID.rendimiento);/////////////cambiar/////////////
};
function activar(data) {
  ...
  $("#contenedor_del_modal").html(`
      <p class="text-danger">¿Está seguro que desea ${estado} el rendimiento: ${arrayID.rendimiento}?</p>
...
function renderDataTable() {
  g__rendimiento_datatables = $("#dt").DataTable({/////////////cambiar/////////////
    data: g__rendimiento,/////////////cambiar/////////////
    response: true,
    destroy: true,
    columns: [/////////////cambiar/////////////
      {data: "id_rendimiento"},
...
function cargar(){
    return new Promise( resolve => {
        fetch('API/V1/rendimiento/read.php')/////////////cambiar/////////////
        .then( r => r.json() )
        .then( r => resolve(r) )
    })
}
...
function eventoCrear(obj){
    fetch('API/V1/rendimiento/create.php', {/////////////cambiar/////////////
        method:'POST',
...
function eventoEditar(data){
    ...
    fetch(`API/V1/rendimiento/update.php?id_rendimiento=${data}`, {/////////////cambiar/////////////
        method:'POST',
