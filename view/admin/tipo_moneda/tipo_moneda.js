...
init();
async function init() {
  g__Tipo_moneda = await cargar();
  g__Tipo_moneda = g__Tipo_moneda.document.records;
  renderDataTable();
}
//-----------------------init-----------------------//
...
//-----------------------Creación de objeto-----------------------//
  function generarObjeto(){
    return{
        tipo_moneda:$("#f_nombre_tipo_moneda").val(),
        activo:1,
    }
  }
  function generarObjetoEdit(data){
      return {
          id_tipo_moneda:data,/////////////cambiar/////////////
          tipo_moneda:$("#f_edit").val(),
          activo:1,
      }
    };
    ...
    function generarObjetoActivar(data){
      ...
      return {
          id_tipo_moneda:arrayID.id_tipo_moneda,/////////////cambiar/////////////
          tipo_moneda:arrayID.tipo_moneda,/////////////cambiar/////////////
          activo:activoTmp,
        }
      };
...
$("#btn_cancelar_tipo_moneda").click(function(){
  $("#f_nombre_tipo_moneda").val("");
});
...
$("#f_nombre_tipo_moneda").keyup(function(){/////////////cambiar/////////////
  if ($("#f_nombre_tipo_moneda").val() == "") {
    $( "#btn_agregar_tipo_moneda" ).prop( "disabled", true );/////////////cambiar/////////////
  }else {
    $( "#btn_agregar_tipo_moneda" ).prop( "disabled", false );/////////////cambiar/////////////
  }
});
//-------------------------------->>
function buscarPorID(data) {
  const found = g__Tipo_moneda.find(element => element.id_tipo_moneda == data);/////////////cambiar/////////////
  return found;
};
function editar(data) {
  $("#contenedor_del_modal").html(`
      <label class="form-label">Nombre del tipo de moneda</label>
    ...
    `);
  let arrayID = buscarPorID(data);
  $("#f_edit").val(arrayID.tipo_moneda);/////////////cambiar/////////////
};
function activar(data) {
  ...
  $("#contenedor_del_modal").html(`
      <p class="text-danger">¿Está seguro que desea ${estado} el tipo de moneda: ${arrayID.tipo_moneda}?</p>
...
function renderDataTable() {
  g__Tipo_moneda_datatables = $("#dt").DataTable({/////////////cambiar/////////////
    data: g__Tipo_moneda,/////////////cambiar/////////////
    response: true,
    destroy: true,
    columns: [/////////////cambiar/////////////
      {data: "id_tipo_moneda"},
...
function cargar(){
    return new Promise( resolve => {
        fetch('API/V1/tipo_moneda/read.php')/////////////cambiar/////////////
        .then( r => r.json() )
        .then( r => resolve(r) )
    })
}
...
function eventoCrear(obj){
    fetch('API/V1/tipo_moneda/create.php', {/////////////cambiar/////////////
        method:'POST',
...
function eventoEditar(data){
    ...
    fetch(`API/V1/tipo_moneda/update.php?id_tipo_moneda=${data}`, {/////////////cambiar/////////////
        method:'POST',
